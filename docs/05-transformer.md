...
## 5.16 DATASET → 末尾ラベル付き DATA ＋ 切替 RESTORE（方式A）

`DATASET`（[01 §1.4.2](01-language-spec.md#142-dataset名前付きデータブロック)）は変換器で処理する（RESTORE 先の行番号＝emit レベルのラベルが要るため、SELECT のような純 desugar ではない）。方針は「メモリ最小・逐次前提」の方式A。

- 全 `DATASET` を収集し、各ブロックに **内部ラベル（RESTORE 先＝先頭 DATA 行）** と **番号 id(≥1)** を割当。`DATA` 行は **出力の末尾**にまとめて置く（各ブロック先頭にラベル）。
- **現在ブロック番号**を保持する整数を**1個だけ**確保（MSX 出力は全変数がグローバル2文字名なので1個で足りる）。MAIN 先頭で `0`（＝未設定）に初期化。
- `READ name INTO t…` を次へ展開：
  ```basic
  IF <cur><>id THEN RESTORE <先頭行>:<cur>=id
  READ t…
  ```
  同ブロックの連続読みはガードで RESTORE を出さず **O(1)**。**別ブロックへ切り替わる時だけ** `RESTORE`。
