...
### Added
- **データファイルをプロジェクトの一員に**：`.txt` / `.dat` / `.csv` / `.mml` / `.asc` / `.tab` の
  テキストデータファイルをプロジェクトツリーの「データファイル」欄で追加・編集できる（欄ヘッダの
  「＋」から追加、拡張子込みで命名）。変換対象外のプレーンテキストとして扱い、`.dsk` / WebMSX 実行
  ペイロードへ**自動同梱**する（Rust で **Shift-JIS＋CRLF＋末尾 EOF(0x1A)** に整形。MSX Disk BASIC の
  `OPEN "NAME.TXT" FOR INPUT` / `LINE INPUT #1` で読める形）。大きなデータを `DATA`(=RAM) でなく
  ディスクへ外出しでき、`OPEN … FOR APPEND` で追記もできる。編集は UTF-8、Shift-JIS 化できない文字は
  保存時に拒否。当面はテキスト／Shift-JIS のみ。
- **「外す」と「削除」の分離**（`.msxb`・データファイル共通）：**⏏ プロジェクトから外す**（ディスクの
  実体は残す／除外リストでフォルダ再読込時の復活を防止・元に戻せる）と **🗑 削除**（OS のゴミ箱へ移動＝
  復元可能・確認あり、`trash` crate）。ブラウザ版（ディスク無し）はどちらもプロジェクト内除去のみ。
- **サイドバーのインライン名前変更**：ファイル名のダブルクリック、または ✎ でその場編集（Enter 確定・
  Esc 取消）。`.msxb`・データファイル共通。
- **エディタの独立タブ化**：📄データファイル・📦ライブラリを「構造化BASIC」タブと**常に併置**（どのタブ
  表示中でも消えない）。各タブはクリックで切替、✕ で閉じてもプロジェクトからは外さない。

### Fixed
- データ／ライブラリ表示中にビルド・実行するとエディタの表示内容をソースとして読み込み、実行が偽エラーに
  なる不具合を修正（`activePath` はソース表示時のみ有効。診断・変換・WebMSX 実行すべてに反映）。
- WebMSX 実行の即時フィードバックを追加（押下で Run タブへ即切替＋「起動中…」表示。データ／ライブラリ
  表示中でも起動と main 実行が分かる）。

### Security
- `esc()` が `"` と `'` もエスケープするよう修正（ツリー行の属性へ差し込むファイル名に引用符が入っても
  属性を破壊・イベントハンドラを注入できないように。ディスク由来の未サニタイズ名を一括で安全化）。
- Tauri コマンド `save_source` にパストラバーサル検証（区切り・親参照・絶対パスを拒否。`read_binary` /
  新規 `trash_file` と同じ規則）を追加。
