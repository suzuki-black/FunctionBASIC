...
test("ライブラリ例: hello / graphics / palette が INCLUDE 解決後に無エラー変換（実機確認済）", () => {
  for (const f of [
    "examples/msx2-lib-hello.msxb",
    "examples/msx2-lib-graphics.msxb",
    "examples/msx2-lib-palette.msxb",
  ]) {
    const { diagnostics } = compileWithIncludes(f);
    assert.deepEqual(
