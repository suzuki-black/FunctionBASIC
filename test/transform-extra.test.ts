...
test("ASM ブロック: STRING$確保＋VARPTRパッチ＋DEFUSR/USR を生成", () => {
  const { msx, diagnostics } = compile(
    `PX% = 100\nASM\n  LD A,(PX%)\n  INC A\n  LD (PX%),A\nEND ASM\nPRINT PX%\n`,
  );
  assert.deepEqual(diagnostics.filter((d) => d.severity === "error"), []);
  assert.match(msx, /STRING\$\(8,0\)/); // 3命令+RET=8バイト確保
  assert.match(msx, /PEEK\(VARPTR\(.+\)\+1\)\+PEEK\(VARPTR\(.+\)\+2\)\*256/); // 実体アドレス
  assert.match(msx, /VARPTR\(A%\)/); // PX% → A%（2文字名）のアドレス取得
