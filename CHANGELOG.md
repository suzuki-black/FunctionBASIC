...
### Added
- **音楽ツール：ディスク保存/読込**：曲（可逆な MML方言 `.mml`）をファイルへ保存・読込する
  「💾 保存(.mml) / 📂 開く(.mml)」。デスクトップはネイティブダイアログ（Rust `save_text_file` /
  `open_text_file`。パスは常にユーザーがダイアログで選択）、ブラウザはダウンロード／`<input type=file>`。
- **音楽ツール：一時保存（自動退避）＋閉じる確認**：編集のたびに退避（デスクトップ＝プロセス内メモリ
  `MusicAutosave` ／ ブラウザ＝localStorage）し、開き直すと**自動復元**（本体終了で消える＝一時的）。
  未保存の変更がある状態で閉じると「変更があります。保存しますか？（保存して閉じる／保存せず閉じる／
  キャンセル）」（Tauri `onCloseRequested`＋確認後 `destroy` ／ ブラウザは `beforeunload`）。
- **音楽ツール：本体言語に追従**：起動時に本体の言語を `?lang=` で受け取り、UI・操作説明・メッセージ・
  ウィンドウタイトルを英/日で完全切替（`open_music_tool` が言語を検証して渡す）。

### Fixed
- **グライドの保存↔読込で形が壊れる（2件）**：
  - `songToMml` がグライドを半音ランへ展開して書き出し、読み戻す構文が無かったため、往復で
    `glideTo`（1音符＋滑り先＝斜線1本）が失われ、バラバラの小音符列（途切れた点線）になっていた。
    → MML方言に **グライド記法 `~<滑り先>`** を追加し展開せず保持。半音ランへの展開は **MSX 出力
    （PLAY/feeder）時のみ**。
  - 単一MML長で表せない音長（16分刻みで 60/108/120/132tick 等）を「同音の再アタック（`C4C16`）」で
    書いていたため、読込で複数音符に分裂し、グライドが末尾断片にだけ付いて斜線がほぼ垂直に化けていた。
    → 方言は **タイ `&`（`C4&16`）** で1音符を維持し読込で長さ加算。MSX PLAY 出力には `&` を出さず
    従来の再アタックのまま（実機互換・合計 tick 同一）。
  - 往復で 1音符／長さ／glideTo をすべて保持（回帰テスト追加、全 446 pass）。`docs/16` に `~`・`&` を明記。
...
### Fixed
- **グライドの音長が最大~10%短くなり3声の同期が崩れる**: 半音ラン展開で刻み長に `floor(dur/n)` を
  使い（`L64` グリッド外の半端が発生）、出力側が単一MML長へ丸めて端数を捨てていた（音符のみ・休符は非対称）。
  刻みを `L64`（3tick）グリッドの倍数へ量子化し、単一長で表せない端数も**連結（tie）で出し切る**よう修正。
  グライドを含め `Song→PLAY`/`Song→MML` で**発音の合計 tick を元の音符長と厳密一致**（回帰テスト追加）。
  **MML化の限界**を `docs/16` に明文化し README 英日にも要約を追記。
- **音楽ツールの PLAY 出力が実機で Illegal function call**: MSX-BASIC の PLAY は `<`/`>`（オクターブシフト）を
  解釈せずエラーになる。オクターブ変更を必ず `O<n>` で出力するよう修正（WebMSX 実機で確認）。範囲外オクターブ
  （O0/O9）も O1–O8 にクランプ。オクターブをまたぐ曲・グライドが実機で鳴らなかった問題を解消。
