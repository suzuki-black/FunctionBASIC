...
test("LINE 相対座標 STEP を受理（順方向）", () => {
  const src = "LINE (0,0)-STEP(10,10),15\nLINE -STEP(8,-36),15,BF\n";
  const { tokens, diagnostics: ld } = tokenize(src);
  const { program, diagnostics: pd } = parse(tokens);
  const r = transform(program);
  assert.deepEqual([...ld, ...pd, ...r.diagnostics].filter((d) => d.severity === "error"), []);
  assert.match(r.code.map((l) => l.text).join("\n"), /STEP\(/); // STEP は素通し（改名されない）
});

test("DATA の任意リテラル(* 等)を受理（順方向）", () => {
  const src = 'DATA *\nDATA "a,b",3\nDATA &H1,&HAC,1\nREAD A$\n';
  const { tokens, diagnostics: ld } = tokenize(src);
  const { program, diagnostics: pd } = parse(tokens);
  const r = transform(program);
  assert.deepEqual([...ld, ...pd, ...r.diagnostics].filter((d) => d.severity === "error"), []);
  const out = r.code.map((l) => l.text).join("\n");
  assert.match(out, /DATA \*/);
  assert.match(out, /DATA "a,b",3/);
});
