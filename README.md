...
- **255-byte line auto-split** — long generated lines are split safely, including long `DATA` lines (split at value/comma boundaries; `READ` sees a flat stream, so the values are unchanged).
...
- **Read-only conversion table** — open it from the project tree to see the generated map: how each long name compresses to its 2-char MSX name (globals and per-function locals), every function's entry line / parameters (incl. `REF`) / return variable, and `BREAK`/`CONTINUE` jump targets. Named compile-time blocks are listed too: `DATASET` (→ its `RESTORE` line and switch id), inline `ASM` (→ load-address variable, one-time-patch guard, `VARPTR`-patched vars, byte count), and `SPRITE` dot-art (→ assigned pattern number and size). Copy it as `.map.json`.
...
- **Error-line navigation** — *Edit → Next error line* (`Cmd/Ctrl+Shift+.`) cycles to the next error line each press and wraps back to the first after the last.
- **Error-location estimator** — MSX often reports a runtime error (e.g. *Illegal function call*) on a line that is not the real cause. *Run → Estimate error location…* takes an error kind + the reported MSX line and algorithmically lists the statements/arguments on that line that could trigger it, mapped back to your original source (heuristic, no AI).
- **Cost analysis (static profiler)** — *Run → Cost analysis (hotspots)…* statically estimates execution cost using **measured turbo-R (R800) per-instruction costs**, without running anything. It shows the per-frame path as a **tree** (the top-level `WHILE` with its state branches split), ranks functions by self/inclusive cost, flags hotspots (`◆`) and heavy ops (`RND`, `LINE…BF`, `PUT SPRITE`, GRP `PRINT`), and honours DCE/reachability so unused `INCLUDE`d library functions (e.g. FAST) are marked, not counted. Inline `ASM` is costed by Z80 T-states with loop detection. Copy the report as text or save it as JSON. See [docs/15](docs/15-cost-analyzer.md).
- **Music tool (MML ⇄ readable `PLAY`)** — a CLI (`music.mjs`) that turns notes into **readable structured-BASIC `PLAY` statements** (one measure per line, `O`/`L` explicit, three voices padded to equal length) and **decompiles** existing `PLAY` statements back into notes. The interchange format is an MML dialect (`@tempo`/`@timesig`/`|` bars); FunctionBASIC owns the "readable `PLAY`" formatting. `import` / `export` / `roundtrip` subcommands stream via stdin/stdout for ecosystem piping. A **piano-roll GUI** (*Run → Music tool*) shares the same core and previews with Web Audio. For in-game music, `--player feeder` emits a **non-blocking BGM feeder** (`LOAD`/`START`/`TICK`/`STOP`) that tops up the PSG queue each frame. **Limits of MML output** (inherent to MSX MML/PSG): finest resolution `L64` (3 ticks, `PPQ=48`); no portamento — a *glide* is approximated as a fast chromatic run (a true continuous sweep needs the PSG-register `SFX_DROP` path); no `<`/`>` octave shift (always emits `O<n>`, since real hardware raises *Illegal function call*); octaves clamped to `O1`–`O8`; PSG 3 voices only (FM not yet). Note **length is never sacrificed for pitch** — glides preserve total ticks exactly, so the three voices stay in sync. See [docs/16 “MML化の限界”](docs/16-music-tool.md).
...
- **Data files (text, bundled)** — the project can hold text **data files** (`.txt` / `.dat` / `.csv` / `.mml` / `.asc` / `.tab`) beside the `.msxb` sources, shown under **Data files** in the project tree (add with the **＋** on that group). They are plain text (not transpiled) and are automatically bundled onto the run payload / `.dsk`, encoded as **Shift-JIS with CRLF line endings and an EOF (`0x1A`) marker** — the form MSX Disk BASIC reads with `OPEN "NAME.TXT" FOR INPUT` / `LINE INPUT #1`. This lets a program keep large data on disk instead of in `DATA` (bypassing the ~28&nbsp;KB BASIC memory limit) and read it a line at a time; `OPEN … FOR APPEND` grows it. Content is edited as UTF-8 and must be Shift-JIS-representable (non-SJIS characters are rejected on save). For now data files are **text/Shift-JIS only**.
- **Remove vs. delete** — every file (both `.msxb` and data files) has two tree actions: **Remove from project** (`⏏`) drops it from the project but **keeps the file on disk** (reversible — it won't be re-added on folder reload), while **Delete** (`🗑`) moves the file to the **OS Trash** after confirmation (recoverable). In the browser editor (no disk) both simply remove the in-project copy.
...
### FAST library A/B — the same shooter, slow vs fast
...
- **255バイト行の自動分割** — 生成された長い行は安全に分割されます。長い `DATA` 行も対象（値／カンマ境界で分割。`READ` は平坦なストリームとして読むので値は不変）。
...
- **変換テーブル（読み取り専用）** — プロジェクトツリーから開くと、生成される対応表を表示：各長い名前が2文字MSX名にどう圧縮されるか（グローバル／関数ごとのローカル）、各関数の先頭行・引数（`REF`含む）・戻り値、`BREAK`/`CONTINUE` の飛び先。コンパイル時の名前付きブロックも掲載：`DATASET`（→ `RESTORE` 先の行・切替番号）、インライン `ASM`（→ 配置アドレス変数・初回パッチガード・`VARPTR`パッチ対象変数・バイト数）、`SPRITE` ドット絵（→ 割当パターン番号・サイズ）。`.map.json` としてコピー可能。
...
- **エラー行ナビゲーション** — *編集 → 次のエラー行へ*（`Cmd/Ctrl+Shift+.`）で押すたびに次のエラー行へ移動し、最後まで行ったら先頭のエラー行へ巡回。
- **エラー箇所の推測** — *Illegal function call* 等、MSXが実行時エラーを「本当の原因ではない行」で報告することがあります。*実行 → エラー箇所を推測…* にエラー種別と報告されたMSX行番号を渡すと、その行にある「原因になり得る命令・引数」をアルゴリズムで列挙し、元の構造化ソース行に対応付けて表示します（推測・AI不使用）。
- **コスト解析（静的プロファイラ）** — *実行 → コスト解析（重い所）…* が、**実機較正した turbo R (R800) の命令別コスト**でソースを静的に解析し（実行なし）、毎フレーム経路を**ツリー**表示（トップレベル `WHILE` を状態分岐ごとに分離）、関数を self/inclusive で順位付け、突出箇所（`◆`）や重い命令（`RND`・`LINE…BF`・`PUT SPRITE`・GRP `PRINT`）を注記します。DCE/到達可能性に対応し、未使用の `INCLUDE` ライブラリ関数（FAST 等）は「未使用」と明示して計上しません。インライン `ASM` は Z80 の T-state ＋ループ検出で概算。結果はテキストでコピー／JSONで保存できます（[docs/15](docs/15-cost-analyzer.md)）。
- **音楽ツール（MML ⇄ 読みやすい `PLAY`）** — 音符を**読みやすい構造化BASICの `PLAY` 文**（1小節1行・`O`/`L`明示・3声を休符で等長化）に書き出し、既存の `PLAY` 文を音符へ**デコンパイル**する CLI（`music.mjs`）。中間フォーマットは MML方言（`@tempo`/`@timesig`/`|`小節）で、「読みやすい `PLAY`」への整形は FunctionBASIC が持ちます。`import`／`export`／`roundtrip` サブコマンドは stdin/stdout で流せ、エコシステム連携（パイプ）に向きます。**ピアノロールGUI**（*実行 → 音楽ツール*）も同じコアを共有し Web Audio で試聴できます。プレイ中BGMには `--player feeder` で**非ブロックのBGMフィーダ**（`LOAD`/`START`/`TICK`/`STOP`。毎フレーム PSG キューに補充）を生成できます。**MML化の限界**（MSX MML/PSG に由来）：最短分解能は `L64`（3tick・`PPQ=48`）／ポルタメント命令が無いため**グライドは半音刻みの高速ランで近似**（真の連続スイープは PSG レジスタ直書きの `SFX_DROP` を使用）／`<`/`>` 非対応で常に `O<n>` 出力（実機は *Illegal function call*）／オクターブは `O1`–`O8` にクランプ／PSG 3声のみ（FM は将来）。**音階のために音長を犠牲にしません**——グライドも合計 tick を厳密保存し3声の同期が崩れません（[docs/16「MML化の限界」](docs/16-music-tool.md)）。
...
- **データファイル（テキスト・同梱）** — プロジェクトは `.msxb` ソースに加えてテキストの**データファイル**（`.txt` / `.dat` / `.csv` / `.mml` / `.asc` / `.tab`）を持てます。プロジェクトツリーの **データファイル** 欄に表示され、その欄の **＋** で追加します。データファイルは変換対象外のプレーンテキストで、実行ペイロード / `.dsk` に自動同梱され、**Shift-JIS・改行 CRLF・末尾 EOF（`0x1A`）** で書き込まれます（MSX Disk BASIC が `OPEN "NAME.TXT" FOR INPUT` / `LINE INPUT #1` で読める形）。これにより大きなデータを `DATA`（=RAM）ではなくディスクに置き、1 行ずつ読めます（BASIC の ~28&nbsp;KB メモリ制限を回避）。`OPEN … FOR APPEND` で追記も可能。編集は UTF-8 で行い、Shift-JIS で表現できない文字は保存時に拒否します。当面は**テキスト／Shift-JIS のみ**対応です。
- **「外す」と「削除」** — すべてのファイル（`.msxb`・データファイル共通）にツリー操作が 2 つあります。**プロジェクトから外す**（`⏏`）はプロジェクトから除くだけで**ディスクの実体は残します**（元に戻せる。フォルダ再読込でも復活しません）。**削除**（`🗑`）は確認のうえファイルを **OS のゴミ箱へ移動**します（復元可能）。ブラウザ版（ディスクなし）ではどちらもプロジェクト内のコピーを取り除くだけです。
