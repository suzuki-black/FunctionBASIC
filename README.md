...
- **音楽ツール（MML ⇄ 読みやすい `PLAY`）** — 音符を**読みやすい構造化BASICの `PLAY` 文**（1小節1行・`O`/`L`明示・3声を休符で等長化）に書き出し、既存の `PLAY` 文を音符へ**デコンパイル**する CLI（`music.mjs`）。中間フォーマットは MML方言（`@tempo`/`@timesig`/`|`小節）で、「読みやすい `PLAY`」への整形は FunctionBASIC が持ちます。`import`／`export`／`roundtrip` サブコマンドは stdin/stdout で流せ、エコシステム連携（パイプ）に向きます。**ピアノロールGUI**（*実行 → 音楽ツール*）も同じコアを共有し Web Audio で試聴できます。プレイ中BGMには `--player feeder` で**非ブロックのBGMフィーダ**（`LOAD`/`START`/`TICK`/`STOP`。毎フレーム PSG キューに補充）を生成できます（[docs/16](docs/16-music-tool.md)）。
