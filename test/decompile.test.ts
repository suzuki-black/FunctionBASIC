...
test("THEN先頭が行番号なら後続:文を伴ってもコメント化（裸の行番号を出さない）", () => {
  const { source } = conv('10 IF A<2 THEN 99:X=1\n20 PRINT 1\n99 END\n');
  assert.match(source, /'\s*\[未対応\] IF A<2 THEN 99:X=1/);
  assert.doesNotMatch(source, /^\s*99\s*$/m);
});
