...
- [ ] **Higher-level structured game-BASIC** (design stage) — syntax that compiles to authentic MSX-BASIC, aimed at making game code read like what it *does*. Structural features lower at zero runtime cost; runtime-heavy ones lean on the inline-Z80 path. Planned:
  - **`SELECT CASE`** (list / `TO` range / `IS` relational / `ELSE`) → a selector-evaluated-once `IF` chain, with an `ON … GOTO` jump-table fast path for dense integers. (There is no `ELSEIF` today, so this becomes the readable multi-way branch.)
  - **`DATASET name … END DATASET` + `READ name INTO …`** — named data blocks with a per-block read position, replacing the opaque global `READ`/`DATA`/`RESTORE` pointer. (MSX keeps a single DATA pointer, so interleaving blocks carries a repositioning cost; sequential use is the fast path.)
  - **`STRUCT`** — struct-of-arrays (`Enemy[i].X`) lowered to parallel per-field MSX arrays.
  ...
  - **`EVENT TIMER`** (→ `ON INTERVAL GOSUB`) and, more cautiously, **`EVENT VBLANK`** — a true VBLANK hook can't safely re-enter the BASIC interpreter, so this is `HALT`-frame-synced or an ASM hook that sets a flag BASIC polls.
...
- [ ] **高水準の構造化ゲームBASIC**（設計段階）— 本物のMSX-BASICへ落ちる構文で、ゲームコードが「何をしているか」そのまま読めることを狙う。構造的な機能はゼロコストで lower、実行負荷の高いものはインラインZ80経路に依存。予定：
  - **`SELECT CASE`**（リスト / `TO` 範囲 / `IS` 関係 / `ELSE`）→ セレクタを一度だけ評価する `IF` チェーン、密整数なら `ON … GOTO` ジャンプテーブルの速い経路。（現状 `ELSEIF` が無いため、これが読みやすい多分岐になる）
  - **`DATASET 名前 … END DATASET` ＋ `READ 名前 INTO …`** — ブロックごとに読み取り位置を持つ名前付きデータブロック。分かりにくい大域 `READ`/`DATA`/`RESTORE` ポインタを置き換える。（MSXのDATAポインタは1本なので、ブロックを交互に読むと再位置決めコストが出る＝順次利用が速い経路）
  - **`STRUCT`** — 構造体を配列群（struct-of-arrays、`Enemy[i].X`）へ lower（フィールドごとにMSX配列へ）。
  ...
  - **`EVENT TIMER`**（→ `ON INTERVAL GOSUB`）と、より慎重に **`EVENT VBLANK`** — 本物のVBLANKフックはBASICインタプリタに安全に再入できないため、`HALT` でフレーム同期するか、ASMフックがフラグを立ててBASICがポーリングする形。
