...
// 未対応コマンド/未定義関数の呼び出しは、原点(行0)でなく「その呼び出し文の行」で
// 報告される（そうでないとエディタのガターにXマーカーが出ず、"エラーはあるのに
// 印がない" という紛らわしい状態になる）。例: RANDOMIZE は MSX-BASIC に無い。
test("未解決の呼び出しは行0でなく実際の行で報告される（ガターマーカー用）", () => {
  const { diagnostics } = compile(`SCREEN 5\nRANDOMIZE\nPRINT "HI"`);
  const e = diagnostics.filter((d) => d.code === "E_UNKNOWN_FUNCTION");
  assert.equal(e.length, 1, JSON.stringify(diagnostics));
  assert.equal(e[0].line, 2, "RANDOMIZE の行(2)で報告されるべき（0ではない）");
  assert.ok(e[0].line > 0, "行0（原点）で報告してはいけない");
});

// 命令中の予約された節/モード/フラグ語は変数化（改名）してはならない。
// 一次資料: MSX Wiki OPEN（FOR OUTPUT/INPUT/APPEND, GRP: デバイス）, PRINT（#n, USING …）, BLOAD（,R）。
test("OPEN … FOR OUTPUT/INPUT/APPEND AS #n のモード語を改名しない", () => {
  assert.match(compile(`OPEN "GRP:" FOR OUTPUT AS #1`).msx, /OPEN "GRP:" FOR OUTPUT AS#1/);
  assert.match(compile(`OPEN "A:F" FOR INPUT AS #1`).msx, /FOR INPUT AS#1/);
  assert.match(compile(`OPEN "A:F" FOR APPEND AS #1`).msx, /FOR APPEND AS#1/);
  assert.match(compile(`OPEN "A:F" FOR OUTPUT AS #1 LEN=256`).msx, /FOR OUTPUT AS#1 LEN=256/);
});
test("PRINT #n, USING の USING をファイル番号の後でも改名しない", () => {
  const a = compile(`PRINT #1, USING "##.#"; V`);
  assert.equal(a.diagnostics.filter((d) => d.severity === "error").length, 0);
  assert.match(a.msx, /PRINT#1,USING ?"##\.#";/);
  assert.match(compile(`PRINT USING "##"; V`).msx, /PRINT USING ?"##";/); // 従来（先頭）も維持
});
