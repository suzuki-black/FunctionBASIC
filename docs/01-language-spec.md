...
- FunctionBASIC の識別子は **大小文字を区別しない**（`col` / `COL` / `Col` は同じ）。正規形は
  **すべて大文字**で、整形時に **コードのトークン（キーワード・変数・関数・`STRUCT` 型名とフィールド・
  `MACRO` 名・`DATASET` 名・ラベル等、あらゆる識別子と数値）を大文字化**する。
  → **PascalCase / camelCase は使わない**（書いても整形で大文字化される。casing は見た目だけで意味を持たない）。
  命名・レイアウトを含む全体のコーディング規約は [13 スタイルガイド](13-style-guide.md)。
